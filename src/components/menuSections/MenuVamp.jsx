...
function MenuVamp({ activeSection }) {
  const { vampColor, setVampColor, vampMaterial, setVampMaterial } =
    useCustomization();
      ...
      <div className="flex gap-2 text-sm">
        <button
          className={[
            ...
            vampMaterial === MATERIAL_TYPES.leather ? 'active' : '',
          ].join(' ')}
          onClick={() => setVampMaterial(MATERIAL_TYPES.leather)}
        >
          Leather
        </button>
        <button
          className={[
            'materialBtn flex items-center gap-1 rounded-full border border-gray-200 bg-white p-2 pl-12 pr-12',
            vampMaterial === MATERIAL_TYPES.canvas ? 'active' : '',
          ].join(' ')}
          onClick={() => setVampMaterial(MATERIAL_TYPES.canvas)}
        >
          Canvas
        </button>
      </div>
              ...
              >
                <button
                  ...
                  className={[
                    ...
                    vampColor.key === color.key ? 'active' : '',
                  ].join(' ')}
                  style={{
                    '--colorSwatch': color.hex,
                  }}
                  onClick={() => setVampColor(color)}
                ></button>
                <span
                  className={[
                    'absolute bottom-[-20px] text-nowrap text-xs',
                    vampColor.key === color.key ? 'block' : 'hidden',
                  ].join(' ')}
                >
                  {color.name}
                </span>
