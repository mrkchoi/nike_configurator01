...
function MenuTip({ activeSection }) {
  const { tipColor, setTipColor, tipMaterial, setTipMaterial } =
    useCustomization();
      ...
      <div className="flex gap-2 text-sm">
        <button
          className={[
            ...
            tipMaterial === MATERIAL_TYPES.leather ? 'active' : '',
          ].join(' ')}
          onClick={() => setTipMaterial(MATERIAL_TYPES.leather)}
        >
          Leather
        </button>
        <button
          className={[
            'materialBtn flex items-center gap-1 rounded-full border border-gray-200 bg-white p-2 pl-12 pr-12',
            tipMaterial === MATERIAL_TYPES.canvas ? 'active' : '',
          ].join(' ')}
          onClick={() => setTipMaterial(MATERIAL_TYPES.canvas)}
        >
          Canvas
        </button>
      </div>
              ...
              >
                <button
                  ...
                  className={[
                    ...
                    tipColor.key === color.key ? 'active' : '',
                  ].join(' ')}
                  style={{
                    '--colorSwatch': color.hex,
                  }}
                  onClick={() => setTipColor(color)}
                ></button>
                <span
                  className={[
                    'absolute bottom-[-20px] text-nowrap text-xs',
                    tipColor.key === color.key ? 'block' : 'hidden',
                  ].join(' ')}
                >
                  {color.name}
                </span>
