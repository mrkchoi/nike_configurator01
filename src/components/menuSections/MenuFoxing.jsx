...
function MenuFoxing({ activeSection }) {
  const { foxingColor, setFoxingColor, foxingMaterial, setFoxingMaterial } =
    useCustomization();
      ...
      <div className="flex gap-2 text-sm">
        <button
          className={[
            ...
            foxingMaterial === MATERIAL_TYPES.leather ? 'active' : '',
          ].join(' ')}
          onClick={() => setFoxingMaterial(MATERIAL_TYPES.leather)}
        >
          Leather
        </button>
        <button
          className={[
            'materialBtn flex items-center gap-1 rounded-full border border-gray-200 bg-white p-2 pl-12 pr-12',
            foxingMaterial === MATERIAL_TYPES.canvas ? 'active' : '',
          ].join(' ')}
          onClick={() => setFoxingMaterial(MATERIAL_TYPES.canvas)}
        >
          Canvas
        </button>
      </div>
              ...
              >
                <button
                  ...
                  className={[
                    ...
                    foxingColor.key === color.key ? 'active' : '',
                  ].join(' ')}
                  style={{
                    '--colorSwatch': color.hex,
                  }}
                  onClick={() => setFoxingColor(color)}
                ></button>
                <span
                  className={[
                    'absolute bottom-[-20px] text-nowrap text-xs',
                    foxingColor.key === color.key ? 'block' : 'hidden',
                  ].join(' ')}
                >
                  {color.name}
                </span>
