...
function MenuQuarter({ activeSection }) {
  const { quarterColor, setQuarterColor, quarterMaterial, setQuarterMaterial } =
    useCustomization();
      ...
      <div className="flex gap-2 text-sm">
        <button
          className={[
            ...
            quarterMaterial === MATERIAL_TYPES.leather ? 'active' : '',
          ].join(' ')}
          onClick={() => setQuarterMaterial(MATERIAL_TYPES.leather)}
        >
          Leather
        </button>
        <button
          className={[
            'materialBtn flex items-center gap-1 rounded-full border border-gray-200 bg-white p-2 pl-12 pr-12',
            quarterMaterial === MATERIAL_TYPES.canvas ? 'active' : '',
          ].join(' ')}
          onClick={() => setQuarterMaterial(MATERIAL_TYPES.canvas)}
        >
          Canvas
        </button>
      </div>
              ...
              >
                <button
                  ...
                  className={[
                    ...
                    quarterColor.key === color.key ? 'active' : '',
                  ].join(' ')}
                  style={{
                    '--colorSwatch': color.hex,
                  }}
                  onClick={() => setQuarterColor(color)}
                ></button>
                <span
                  className={[
                    'absolute bottom-[-20px] text-nowrap text-xs',
                    quarterColor.key === color.key ? 'block' : 'hidden',
                  ].join(' ')}
                >
                  {color.name}
                </span>
