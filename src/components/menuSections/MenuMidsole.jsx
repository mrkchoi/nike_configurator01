...
function MenuMidsole({ activeSection }) {
  const { midsoleColor, setMidsoleColor } = useCustomization();
              ...
              >
                <button
                  ...
                  className={[
                    ...
                    midsoleColor.key === color.key ? 'active' : '',
                  ].join(' ')}
                  style={{
                    '--colorSwatch': color.hex,
                  }}
                  onClick={() => setMidsoleColor(color)}
                ></button>
                <span
                  className={[
                    'absolute bottom-[-20px] text-nowrap text-xs',
                    midsoleColor.key === color.key ? 'block' : 'hidden',
                  ].join(' ')}
                >
                  {color.name}
                </span>
